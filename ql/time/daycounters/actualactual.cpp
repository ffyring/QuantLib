    ...
    namespace {
        ...
        */
        ...
        std::vector<Date> getListOfPeriodDatesIncludingQuasiPayments(
            ...
            // We need to find the first quasi-coupon date to determine
            // whether it is a long first.
            Date issueDate = schedule.date(0);
            Date firstCoupon = schedule.date(1);
            Date lastRegular = schedule.specificLastDate() != Date() ? 
                schedule.specificLastDate() : schedule.endDate();
            Date firstQuasi = lastRegular;
            Integer i = 0;
            while (firstQuasi != firstCoupon) {
                firstQuasi = schedule.calendar().advance(lastRegular,
                    -(++i) * schedule.tenor(),
            ...
            Date notionalCoupon =
                schedule.calendar().advance(firstQuasi,
                    -schedule.tenor(),
                    schedule.businessDayConvention(),
                    schedule.endOfMonth());
    ...
    boost::shared_ptr<DayCounter::Impl>
        ...
        case Bond:
            if (!schedule.empty())
                ...
                return boost::shared_ptr<DayCounter::Impl>(new Old_ISMA_Impl);
        case ISDA:
        case Historical:
        case Actual365:
    ...
    Time ActualActual::Old_ISMA_Impl::yearFraction(const Date& d1,
        ...
        if (d2 <= refPeriodEnd) {
            ...
            if (d1 >= refPeriodStart) {
                ...
                // here refPeriodStart is the nboost (maybe notional)
                // payment date and refPeriodEnd is the second nboost
